...
export const WhatsAppButton = () => {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (settings && settings.whatsapp_number) {
          setNumber(settings.whatsapp_number.replace(/\D/g, ''));
          setIsVisible(true);
        }
      } catch (error) {
  ...
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-50 flex items-center justify-center w-14 h-14 bg-[#25D366] text-white rounded-full shadow-lg hover:bg-[#1EBE5A] hover:scale-110 transition-all duration-300 animate-in fade-in zoom-in group"
